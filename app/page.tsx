...
export default function BankApp() {
  ...
  function startRegister() {
    ...
    if (!acceptTerms) { setError("You must accept the Terms and License"); return; }
  ...
  function completeVerify() {
    ...
    setTimeout(() => {
      const newUser: User = {
        ...
        balance: 1000, bonus: 500, unlocked: false, txs: [], createdAt: new Date().toISOString(), verified: true,
      };
  ...
  function unlockBonus() {
    ...
    setLoading(true);
    setTimeout(() => {
      updateCurrent({ unlocked: true, balance: current.balance + current.bonus, bonus: 0 });
      setShowPin(false); setPin(""); setLoading(false);
      setMsg("Bonus unlocked and credited");
    ...
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(165deg,#070b14,#0f172a 50%,#0c1220)", color: "#e2e8f0", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "system-ui,-apple-system,sans-serif" }}>
        ...
        <div style={{ marginTop: 32, fontSize: 12, color: "#64748b", textAlign: "center", lineHeight: 1.7 }}>
          🔒 256-bit encrypted · Multi-user · Licensed<br />
          <a href="https://github.com/pointgoddesscc-sketch/pse-bank-license-compliance" target="_blank" rel="noreferrer" style={{ color: "#38bdf8" }}>View License & Compliance</a>
        </div>
      </div>
    );
  }
        ...
        <div style={{ maxWidth: 420, margin: "0 auto" }}>
          ...
          <div style={card}>
            ...
            <label style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: 13, color: "#94a3b8", marginBottom: 18, cursor: "pointer" }}>
              ...
              <span>I accept the Terms, License, and confirm I am 18+. I understand PS&E Bank is not FDIC-insured.</span>
            </label>
  ...
  const navItems: [View, string][] = [
    ["dashboard", "Home"], ["send", "Send"], ["receive", "Receive"], ["topup", "Top Up"],
    ["crypto", "Crypto"], ["history", "Activity"], ["more", "More"],
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070b14", color: "#e2e8f0", fontFamily: "system-ui,-apple-system,BlinkMacSystemFont,sans-serif", paddingBottom: 72 }}>
      ...
      <nav style={{ display: "flex", overflowX: "auto", gap: 2, padding: "8px 10px 0", background: "#0a0f1a", borderBottom: "1px solid #1e293b", scrollbarWidth: "none" }}>
        {navItems.map(([id, label]) => (
          <button key={id} onClick={() => setView(id)} style={{
            padding: "10px 14px", borderRadius: "10px 10px 0 0", border: "none", fontSize: 12.5, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap",
            background: view === id ? "#111827" : "transparent", color: view === id ? "#38bdf8" : "#64748b",
          }}>{label}</button>
        ))}
      </nav>
      ...
      <main style={{ padding: "18px 14px", maxWidth: 440, margin: "0 auto" }}>
        ...
        {view === "dashboard" && (
          <div>
            ...
            {!current.unlocked && current.bonus > 0 && (
              <div style={{ ...card, borderColor: "rgba(124,58,237,0.5)", background: "linear-gradient(145deg,#1e1b4b,#111827)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontSize: 11, color: "#a78bfa", letterSpacing: "0.06em", fontWeight: 600 }}>WELCOME BONUS</div>
                    <div style={{ fontSize: 24, fontWeight: 800, color: "#c4b5fd" }}>${current.bonus.toLocaleString()}</div>
                  </div>
                  <button onClick={() => setShowPin(true)} style={{ background: "linear-gradient(135deg,#7c3aed,#6d28d9)", color: "#fff", border: "none", borderRadius: 12, padding: "11px 18px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Unlock</button>
                </div>
              </div>
            )}
            ...
            {showPin && (
              <div style={card}>
                <div style={{ fontWeight: 700, marginBottom: 12 }}>Enter PIN (any 4+ digits)</div>
                <input type="password" style={{ ...inp, textAlign: "center", letterSpacing: "0.35em", fontSize: 20 }} value={pin} onChange={e => setPin(e.target.value)} maxLength={6} />
                <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
                  <button style={{ ...btnG, flex: 1 }} onClick={() => setShowPin(false)}>Cancel</button>
                  <button style={{ ...btnP, flex: 1 }} onClick={unlockBonus} disabled={loading || pin.length < 4}>{loading ? "..." : "Confirm"}</button>
                </div>
              </div>
            )}
            ...
            <div style={{ marginTop: 4, marginBottom: 6 }}>
              ...
              <div style={{ ...card, marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: "#0f172a", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 12, color: "#0ea5e9", border: "1px solid #1e293b" }}>CH</div>
                  <div><div style={{ fontWeight: 600, fontSize: 14 }}>Chase</div><div style={{ fontSize: 12, color: "#64748b" }}>Chase ···· 2847</div></div>
                </div>
                <span style={{ fontSize: 11, color: "#34d399", background: "rgba(16,185,129,0.12)", padding: "4px 10px", borderRadius: 999, fontWeight: 600 }}>Connected</span>
              </div>
              <div style={{ ...card, marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: "#0f172a", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 11, color: "#ef4444", border: "1px solid #1e293b" }}>BA</div>
                  <div><div style={{ fontWeight: 600, fontSize: 14 }}>Bank of America</div><div style={{ fontSize: 12, color: "#64748b" }}>Bank of America ···· 9034</div></div>
                </div>
                <span style={{ fontSize: 11, color: "#34d399", background: "rgba(16,185,129,0.12)", padding: "4px 10px", borderRadius: 999, fontWeight: 600 }}>Connected</span>
              </div>
        ...
        {view === "crypto" && (
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              <button style={{ flex: 1, padding: "10px", borderRadius: 12, border: "none", fontWeight: 650, fontSize: 13, cursor: "pointer", background: "#1e293b", color: "#94a3b8" }}>Giftcard</button>
              <button style={{ flex: 1, padding: "10px", borderRadius: 12, border: "none", fontWeight: 650, fontSize: 13, cursor: "pointer", background: "linear-gradient(135deg,#0ea5e9,#0284c7)", color: "#fff" }}>Crypto</button>
            </div>
            <div style={card}>
              ...
              <div style={{ background: "#0f172a", borderRadius: 12, padding: 14, marginBottom: 14 }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 15 }}>1 {selectedCrypto} = ${cryptoRates[selectedCrypto].toLocaleString("en-US", { minimumFractionDigits: 2 })} USD <span style={{ color: "#34d399", fontSize: 12, marginLeft: 8 }}>↑ +1.24% (24h)</span></div>
              </div>
              ...
              <button style={btnP} onClick={() => setMsg("Trade feature coming soon — balance remains $1,000.00")}>Trade Crypto</button>
            </div>
          </div>
        )}
            ...
            <div style={card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                <div style={{ background: "#0f172a", borderRadius: 12, padding: 14 }}>
                  ...
                  <div style={{ fontSize: 22, fontWeight: 800, color: "#38bdf8" }}>92.5%</div>
                </div>
                <div style={{ background: "#0f172a", borderRadius: 12, padding: 14 }}>
                  <div style={{ fontSize: 11, color: "#94a3b8" }}>Total Value</div>
                  <div style={{ fontSize: 22, fontWeight: 800 }}>${(gcRange * 0.925).toFixed(2)} USD</div>
                </div>
              </div>
              <button style={btnP} onClick={() => setMsg("Giftcard payout calculated — balance remains $1,000.00")}>Calculate Payout</button>
            </div>
          </div>
        )}
        ...
        {view === "cards" && (
          <div>
            ...
            <div style={{ ...card, background: "linear-gradient(145deg,#0c1220,#1e293b)" }}>
              <div style={{ fontSize: 12, color: "#38bdf8", fontWeight: 600, marginBottom: 6 }}>PREMIUM US BANKING</div>
              <div style={{ background: "linear-gradient(135deg,#0ea5e9,#0284c7)", borderRadius: 14, padding: 20, marginBottom: 18, color: "#fff", boxShadow: "0 12px 32px rgba(14,165,233,0.3)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 28 }}>
                  <div style={{ fontWeight: 800, fontSize: 16, letterSpacing: "0.08em" }}>PS&E ELITE</div>
        ...
        {view === "history" && (
          <div>
            ...
            {current.txs.length === 0 ? (
              <div style={{ ...card, textAlign: "center", color: "#64748b", padding: 40 }}>No transactions yet</div>
              ...
              <div key={tx.id} style={{ ...card, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px" }}>
                <div>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>{tx.type === "received" ? `From ${tx.from}` : `To ${tx.to}`}</div>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                ...
                { id: "license" as View, label: "License", sub: "Compliance docs", icon: "📋" },
              ].map(item => (
        ...
        {view === "license" && (
          <div>
            <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 16 }}>License & Compliance</h2>
            <div style={card}>
              <div style={{ fontWeight: 700, marginBottom: 12 }}>PS&E Bank — PSE-LIC-2026-001</div>
              <div style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.7 }}>
                <strong style={{ color: "#e2e8f0" }}>Entity:</strong> Pacers Sport & Entertainment Management / PS&E Bank<br /><br />
                <strong style={{ color: "#e2e8f0" }}>Office:</strong> 10883 Pearl Road, Ste. 203, Strongsville, OH 44136<br /><br />
                <strong style={{ color: "#fbbf24" }}>Not FDIC-insured.</strong>
              </div>
              <a href="https://github.com/pointgoddesscc-sketch/pse-bank-license-compliance" target="_blank" rel="noreferrer" style={{ display: "inline-block", marginTop: 14, color: "#38bdf8", fontWeight: 600, fontSize: 14 }}>View Official License Repository →</a>
            </div>
          </div>
        )}
      </main>
